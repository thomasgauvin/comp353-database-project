...
<html>
    ...
    <body>
        <div class="d-flex">
        <nav class="flex-column sidebar justify-content-start navbar navbar-dark bg-dark" style="width:350px;">
            ...
            <?php
                require('./control/h_get_all_users_in_group.php');
                    for($x = 0; $x < count($h_get_all_users_in_group__groupUsers); $x++){
                        $groupUserID = array_keys($h_get_all_users_in_group__groupUsers)[$x];
                        $groupUser = $h_get_all_users_in_group__groupUsers[$groupUserID];
                        $groupUserFName = $groupUser["FName"];
                        $groupUserLName = $groupUser["LName"];
                        $start_message_button__conversationUserID = $groupUser["UserID"];
                        print("<li class='nav-item navbar-text'>{$groupUserFName} {$groupUserLName}");
